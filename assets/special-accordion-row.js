...
class SpecialAccordionRow {
  ...
  constructor(details) {
    ...
    this.row = details.closest('[data-special-row]');
    this.summary = details.querySelector('.special-accordion-summary');
    this.revealsWrapper = details.querySelector('.special-row__reveals');
    this.reveals = Array.from(details.querySelectorAll('.special-row__reveal'));
    this.detailsContent = details.querySelector('.special-accordion-content');
  ...
  #clearHoverState = () => {
    this.hoveredSide = null;
    this.summary?.querySelectorAll('.special-accordion-summary__heading').forEach((heading) => {
  ...
  #resolveSideFromEvent(event) {
    ...
    if (this.reveals.length <= 1) return this.reveals[0]?.dataset.slot === 'right' ? 'right' : 'left';
   ...
   */
  #setActiveSide(side) {
    const availableSlot = this.reveals.some((reveal) => reveal.dataset.slot === side);
  ...
  #syncActiveReveal() {
    ...
    if (!this.reveals.some((reveal) => reveal.dataset.slot === desiredSide)) {
      desiredSide = this.reveals[0].dataset.slot === 'right' ? 'right' : 'left';
    }

    this.reveals.forEach((reveal) => {
      const isActive = reveal.dataset.slot === desiredSide;
      reveal.dataset.active = isActive ? 'true' : 'false';
    });
    ...
    this.#updateWrapperHeight();
    const activeReveal = this.reveals.find((reveal) => reveal.dataset.active === 'true');
    if (activeReveal) {
      this.#initializeRevealAssets(activeReveal);
    }
  }

  #updateWrapperHeight() {
    if (!this.revealsWrapper) return;
    const activeReveal = this.reveals.find((reveal) => reveal.dataset.active === 'true');
    if (!activeReveal) return;

    const height = activeReveal.scrollHeight;
    this.revealsWrapper.style.setProperty('--special-row-min-height', `${height}px`);
  }

  /**
   * @param {HTMLElement} reveal
   */
  #initializeRevealAssets(reveal) {
    if (this.lazyInitialized.has(reveal)) return;
    this.lazyInitialized.add(reveal);

    const lazyNodes = reveal.querySelectorAll('[data-lazy-preset]');
  ...
  async #loadModule(moduleName) {
    if (SpecialAccordionRow.loadedModules.has(moduleName)) return;
    ...
    try {
      ...
      SpecialAccordionRow.loadedModules.add(moduleName);
    } catch (error) {
  ...
  #updateEmptyState() {
    ...
    const hasContent = this.reveals.some((reveal) => reveal.dataset.empty !== 'true');
...
const INITIALIZED_KEY = Symbol('specialAccordionRow');

/**
 * @param {ParentNode} scope
 */
function init(scope = document) {
  const detailsList = scope.querySelectorAll('[data-special-accordion]');
  ...
  detailsList.forEach((details) => {
    ...
    details[INITIALIZED_KEY] = new SpecialAccordionRow(details);
  });
}
